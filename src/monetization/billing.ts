import chalk from "chalk";
import { PLANS, formatPlan, type PlanTier } from "./plans.js";
import {
  getMonthlyUsage,
  getCurrentPlan,
  setPlan,
  resetBillingCycle,
  recordOverage,
} from "./usage-tracker.js";
import { getSummary } from "../utils/analytics.js";

const KOFI_BASE = "https://ko-fi.com";
const UPGRADE_LINKS: Record<string, string> = {
  pro: `${KOFI_BASE}/bennyco?tx=pro`,
  team: `${KOFI_BASE}/bennyco?tx=team`,
  enterprise: `${KOFI_BASE}/bennyco?tx=enterprise`,
};

export function formatPlanStatus(): string {
  const state = getCurrentPlan();
  const { used, limit, plan } = getMonthlyUsage();
  const p = PLANS[plan];

  const lines: string[] = [];
  lines.push(chalk.bold("\n=== Benny Plan 狀態 ===\n"));

  const tierColors: Record<PlanTier, (s: string) => string> = {
    free: chalk.gray,
    pro: chalk.cyan,
    team: chalk.green,
    enterprise: chalk.magenta,
  };

  const tierColor = tierColors[plan] ?? chalk.white;
  lines.push(`  計劃: ${tierColor(p.name)}`);

  if (p.priceMonthly === 0) {
    lines.push(`  價格: ${chalk.green("免費")}`);
  } else if (p.priceMonthly > 0) {
    lines.push(`  價格: ${chalk.cyan(`$${p.priceMonthly}/月`)}`);
  } else {
    lines.push(`  價格: ${chalk.magenta("企業定制")}`);
  }

  if (p.tokenLimitMonthly > 0) {
    const pct = Math.min(100, Math.round((used / limit) * 100));
    const barLen = 20;
    const filled = Math.round((barLen * pct) / 100);
    const bar = "█".repeat(filled) + "░".repeat(barLen - filled);
    const colorFn = pct >= 90 ? chalk.red : pct >= 70 ? chalk.yellow : chalk.green;
    lines.push(`\n  配額: ${colorFn(bar)} ${pct}%`);
    lines.push(`       ${used.toLocaleString()} / ${limit.toLocaleString()} tokens`);
    const remaining = Math.max(0, limit - used);
    lines.push(`       剩餘: ${(remaining / 1000).toFixed(0)}k tokens`);
  } else {
    lines.push(`\n  配額: ${chalk.green("∞ 無限")}`);
  }

  lines.push(chalk.bold("\n\n=== 功能 ===\n"));
  for (const feat of p.features) {
    lines.push(`  ${chalk.green("✓")} ${feat}`);
  }

  const nextTier = plan === "free" ? "pro" : plan === "pro" ? "team" : null;
  if (nextTier) {
    lines.push(chalk.bold("\n\n=== 升級建議 ===\n"));
    const next = PLANS[nextTier];
    lines.push(`  ${chalk.cyan("→")} 升級到 ${next.name}: ${formatPlan(next)}`);
    lines.push(`    運行 ${chalk.green("benny upgrade")} 查看詳細`);
  }

  lines.push("");
  return lines.join("\n");
}

export function formatUpgradeOptions(currentTier: PlanTier): string {
  const lines: string[] = [];
  lines.push(chalk.bold("\n=== Benny 升級計劃 ===\n"));
  lines.push(chalk.gray("  所有付費計劃包含當前計劃的全部功能\n"));

  const tiers: PlanTier[] = ["pro", "team", "enterprise"];
  for (const tier of tiers) {
    const p = PLANS[tier];
    const isCurrent = tier === currentTier;
    const isDowngrade = tiers.indexOf(tier) <= tiers.indexOf(currentTier);

    lines.push(chalk.bold(`\n【 ${p.name} 】`));
    if (isCurrent) {
      lines.push(chalk.green("  ✓ 當前計劃"));
    } else if (isDowngrade) {
      lines.push(chalk.gray(`  (已解鎖更高計劃)`));
    }
    lines.push(`  價格: ${p.priceMonthly > 0 ? chalk.cyan(`$${p.priceMonthly}/月`) : chalk.magenta("企業定制")}`);

    const tokens = p.tokenLimitMonthly < 0 ? "無限制" : `${(p.tokenLimitMonthly / 1000).toLocaleString()}k tokens/月`;
    lines.push(`  配額: ${tokens}`);

    for (const feat of p.features) {
      const prefix = isCurrent ? chalk.gray("  · ") : chalk.green("  · ");
      lines.push(`${prefix}${feat}`);
    }

    if (!isCurrent && UPGRADE_LINKS[tier]) {
      lines.push(chalk.cyan(`\n  → 支持連結: ${UPGRADE_LINKS[tier]}`));
    }
    lines.push("");
  }

  lines.push(chalk.gray("  付款後運行以下命令激活:\n"));
  lines.push(chalk.green("    benny plan --activate <tier>\n"));
  lines.push(chalk.gray("  (目前為模擬激活，用於演示)\n"));

  return lines.join("\n");
}

export async function activatePlan(tierArg: string): Promise<void> {
  const tierMap: Record<string, PlanTier> = {
    free: "free", pro: "pro", team: "team", enterprise: "enterprise",
  };
  const tier = tierMap[tierArg.toLowerCase()];
  if (!tier) {
    console.error(chalk.red(`無效的計劃: ${tierArg}。可用: free, pro, team, enterprise`));
    process.exit(1);
  }

  const current = getCurrentPlan();
  setPlan(tier);
  resetBillingCycle();

  if (tier !== current) {
    console.log(chalk.green(`\n✓ 計劃已更新: ${PLANS[current].name} → ${PLANS[tier].name}`));
    console.log(chalk.gray("  配額已重置，請重新運行命令。\n"));
  } else {
    console.log(chalk.gray(`\n  已經是 ${PLANS[tier].name} 計劃。\n`));
  }
}
