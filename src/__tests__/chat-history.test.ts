...
describe("chat-history", () => {
  ...
  describe("listSessions", () => {
    it("should return empty array when no history file exists", () => {
      vi.mocked(fs.existsSync).mockImplementation((p) => {
        if (p === "/tmp/test-home/.benny") return true;
        if (p === HISTORY_FILE) return false;
        return false;
      });
  ...
  describe("loadSession", () => {
    it("should return null for non-existent session", () => {
      ...
      vi.mocked(fs.readFileSync).mockReturnValue(
        JSON.stringify({ sessions: [], currentSessionId: null })
      );
    ...
    it("should return session when found", () => {
      ...
      vi.mocked(fs.readFileSync).mockReturnValue(
        JSON.stringify({ sessions: [mockSession], currentSessionId: null })
      );
