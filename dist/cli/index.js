...
program
    ...
    if (defaultModel === "qwen-plus") {
        ...
        if (apiKey)
            config.apiKeys.TONGYI_API_KEY = apiKey;
    }
        ...
        if (apiKey)
            config.apiKeys.WENXIN_API_KEY = apiKey;
        if (secretKey)
            config.apiKeys.WENXIN_SECRET_KEY = secretKey;
    }
    else {
        const apiKey = (await question(`Kimi API Key (${process.env.KIMI_API_KEY ? '[已设置，忽略]' : '[未设置，直接回车跳过]'}: `)).trim();
        if (apiKey)
            config.apiKeys.KIMI_API_KEY = apiKey;
    }
    saveConfig(config);
...
program
    ...
    try {
        const optimized = await optimizeCode(code, language, { style: opts.style });
        console.log(chalk.green("\n优化结果:"));
...
program
    ...
    const modelsToUse = opts.model === "all"
        ? AVAILABLE_MODELS
        : AVAILABLE_MODELS.filter((m) => opts.model.split(",").map((s) => s.trim()).includes(m.provider));
