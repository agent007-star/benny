...
export async function optimizeCode(code, language, options = {}) {
    const opts = {
        style: "general",
        addComments: true,
        fixNaming: true,
...
export async function addChineseComments(code, language) {
    const model = selectModel();
    const messages = [
        {
            role: "system",
            content: "你是一个中文代码注释助手。请为代码添加中文注释，解释关键逻辑。直接返回带注释的代码，不要额外说明。",
        },
        {
            role: "user",
...
export async function translateCode(code, fromLang, toLang) {
    const model = selectModel();
    const messages = [
        {
            role: "system",
