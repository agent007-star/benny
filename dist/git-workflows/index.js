...
async function analyzeRisks(diff) {
    const risks = [];
    const patterns = [
...
export async function reviewChanges(git) {
    const diff = await git.diff(["--staged"]);
    if (!diff)
        return "没有暂存的更改可供审查。";
...
export async function smartCommit(git, message, options = {}) {
    const opts = {
        autoCommit: false,
        commitPrefix: "feat:",
        ...options,
...
export function createGitWorkflow(gitDir) {
