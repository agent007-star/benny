...
function detectLanguageFromContent(code) {
    if (/^\s*import\s+.*from\s+['"][^'"]+['"]/m.test(code) && (code.includes(": ") || code.includes("const ") || code.includes("let ")))
        return "typescript";
    if (/def\s+\w+\s*\(/m.test(code) && !code.includes("func "))
        return "python";
    if (/func\s+\w+\s*\(/m.test(code) || /package\s+\w+/m.test(code))
        return "go";
    if (/fn\s+\w+\s*\(/m.test(code) || /let\s+mut\s+\w+/m.test(code))
        return "rust";
    if (/public\s+(static\s+)?(void|class|interface)/m.test(code) || /System\.out\.println/m.test(code))
        return "java";
    if (/#include\s*</m.test(code) || /int\s+main\s*\(/m.test(code))
        return "cpp";
    return "text";
}
...
async function callTool(name, args) {
    ...
    if (name === "analyze_git_changes") {
        try {
            ...
            const git = simpleGit(args.repo_path || ".");
    ...
    const language = args.language || detectLanguage("code.txt", code);
    const style = args.style || "general";
    ...
    try {
        ...
        const request = { model, messages, temperature: 0.7 };
    ...
    if (method === "tools/call") {
        ...
        try {
            ...
            return {
                jsonrpc: "2.0",
                id,
                error: { code: -32603, message: `Tool error: ${err.message}` },
            };
        }
    }
